...
album.ratio      <- data.train[, .(album_ratio = mean(is_listened), count = .N),          by = album_id]
...
user.album.ratio    <- data.train[, .(user_album_ratio = mean(is_listened), count = .N),    by = .(user_id, album_id)]
...
album.ratio    <- album.ratio[count    >= 30, ]
...
user.album.ratio   <- user.album.ratio[count    >= 30, ]
...
data.full <- merge(data.full, album.ratio,      all.x = TRUE, sort = F, by = "album_id")
...
data.full <- merge(data.full, user.album.ratio,    all.x = TRUE, sort = F, by = c("user_id", "album_id"))
...
data.full[is.na(album_ratio),    album_ratio        := mean(data.train$is_listened)]
...
data.full[is.na(user_album_ratio),    user_album_ratio    := user_ratio_full]
data.full[is.na(user_context_ratio), user_context_ratio := user_ratio_full]

# droping objects from memory
rm(list = c("data.train", "genre.ratio", "song.ratio", "artist.ratio", "context.ratio",
