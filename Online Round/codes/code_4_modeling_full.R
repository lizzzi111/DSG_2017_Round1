...
# loading data
data.full <- as.data.frame(fread(file.path(data.folder, "data_full.csv"), sep = ",", dec = ".", header = T))
...
equation <- as.formula(is_listened ~ user_ratio_flow + user_ratio_full + user_age + user_gender + 
                         ...
                         media_duration + weekday + time_diff_release_listen)
...
# saving predictions
write.table(xg.pred.valid, file = file.path("pred_valid", "xg_full_0527.csv"), quote = F, sep = ",", dec = ".")
#write.table(rf.pred.valid, file = file.path("pred_valid", "rf_full_0527.csv"), quote = F, sep = ",", dec = ".")
write.table(lr.pred.valid, file = file.path("pred_valid", "lr_full_0527.csv"), quote = F, sep = ",", dec = ".")
...
# creating submission
submit(xg.pred.unknown, data = data.unknown, folder = "pred_unknown", file = "xg_full_0527.csv")
#submit(rf.pred.unknown, data = data.unknown, folder = "pred_unknown", file = "rf_full_0527.csv")
submit(lr.pred.unknown, data = data.unknown, folder = "pred_unknown", file = "lr_full_0527.csv")