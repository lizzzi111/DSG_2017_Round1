...
compute_features_dt <- function(train, test) {
  ...
  test_dt$user_song_plays   <- merge(test_dt[, c("sample_id", "media_id")],  user_plays_song,   sort = F, all.x = T)$is_listened
  test_dt$user_album_plays  <- merge(test_dt[, c("sample_id", "album_id")],  user_plays_album,  sort = F, all.x = T)$is_listened
  test_dt$user_artist_plays <- merge(test_dt[, c("sample_id", "artist_id")], user_plays_artist, sort = F, all.x = T)$is_listened
  test_dt$user_genre_plays  <- merge(test_dt[, c("sample_id", "genre_id")],  user_plays_genre,  sort = F, all.x = T)$is_listened
  test_dt$user_song_skips   <- merge(test_dt[, c("sample_id", "media_id")],  user_skips_song,   sort = F, all.x = T)$is_skipped
  test_dt$user_album_skips  <- merge(test_dt[, c("sample_id", "album_id")],  user_skips_album,  sort = F, all.x = T)$is_skipped
  test_dt$user_artist_skips <- merge(test_dt[, c("sample_id", "artist_id")], user_skips_artist, sort = F, all.x = T)$is_skipped
  test_dt$user_genre_skips  <- merge(test_dt[, c("sample_id", "genre_id")],  user_skips_genre,  sort = F, all.x = T)$is_skipped
